...
class VectorStore:
    ...
    def add_documents(self, documents: List[Dict[str, Any]], vector_id: str = None):
        ...
        event_bus.publish(
            VectorStoreEvent.DOCUMENT_ADDED, {"ids": added_documents_ids, "documents": documents}
        )
    ...
    def update_document(self, document_id: str, text: str, metadata: dict):
        ...
        response = client.embeddings.create(
            input=text, model=os.getenv("GPT_EMBEDDING_MODEL", "text-embedding-3-small")
        )
        ...
        event_bus.publish(
            ...
            {"id": document_id, "text": text, "metadata": metadata},
        )
        event_bus.publish(VectorStoreEvent.COLLECTION_MODIFIED)
    ...
    def update_document_status(
        self, document_id: str, status: str, edited_text: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            event_bus.publish(
                VectorStoreEvent.DOCUMENT_UPDATED,
                {"id": document_id, "text": edited_text, "metadata": metadata},
            )
            event_bus.publish(VectorStoreEvent.COLLECTION_MODIFIED)
