...
class CrawledDomainRepository(Repository[CrawledDomain]):
    ...
    def get_by_domain(self, domain: str) -> Optional[CrawledDomain]:
        return (
            self.db.query(CrawledDomain).filter(CrawledDomain.domain == domain).first()
        )
...
class DocumentRepository(Repository[Document]):
    ...
    def get_by_uri(self, uri: str, agent_type: Optional[str] = None) -> List[Document]:
    ...
    def get_by_domain(
        self, domain_id: int, agent_type: Optional[str] = None
    ) -> List[Document]:
    ...
    def search_by_title(
        self, query: str, agent_type: Optional[str] = None
    ) -> List[Document]:
    ...
    def get_by_agent_type(self, agent_type: str) -> List[Document]:
        """Get documents by agent type (voice_agent, text_agent, or both)"""
        return (
            self.db.query(Document)
            .filter(
                (Document.agent_type == agent_type) | (Document.agent_type == "both")
            )
            .all()
        )
...
class CrawlJobsRepository(Repository[CrawlJobs]):
    ...
    def complete_job(
        self, job_id: str, status: Optional[dict] = None
    ) -> Optional[CrawlJobs]:
    ...
    def get_jobs_by_domain(self, domain: str) -> List[CrawlJobs]:
        ...
        return (
            self.db.query(CrawlJobs)
            .filter(CrawlJobs.init_url.like(f"%{domain}%"))
            .all()
        )
    ...
    def get_recent_jobs(self, limit: int = 10) -> List[CrawlJobs]:
        ...
        return (
            self.db.query(CrawlJobs)
            .order_by(CrawlJobs.started_at.desc())
            .limit(limit)
            .all()
        )
