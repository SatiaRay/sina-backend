...
async def update_knowledge(request: UpdateKnowledgeRequest):
    ...
    **نمونه خروجی:**
    ...
    try:
        ...
        file_name = url_parsed.netloc + url_parsed.path.replace("/", "_").replace(
            ".", "_"
        )
...
async def delete_url_data_from_chroma(url: str) -> int:
    ...
    try:
        ...
        url_chunk_ids = []
        for i, (doc_id, doc_metadata) in enumerate(
        ...
        for doc in docs:
            ...
            if doc["text"].strip():
                source_groups[source_id]["chunks"].append(
                    Chunk(text=doc["text"], metadata=doc["metadata"])
                )
    ...
    try:
        ...
        for doc in docs:
            if doc["metadata"].get("source", "") == url and doc["text"].strip():
                chunks.append(Chunk(text=doc["text"], metadata=doc["metadata"]))
...
async def get_crawled_files():
    ...
    try:
        ...
        if not data_dir.exists():
            return JSONResponse(
                content={"files": []}, media_type="application/json; charset=utf-8"
            )
        ...
        return JSONResponse(
            content={"files": file_info}, media_type="application/json; charset=utf-8"
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def search_vector_docs(
    request: VectorSearchRequest = Body(
        ..., example={"question": "ساتیا چیست؟", "limit": 5}
    )
):
    """
    ...
    try:
        ...
        return {"query": request.question, "results": results, "count": len(results)}
        ...
        raise HTTPException(
            status_code=500,
            detail={"message": "Failed to search vector store", "error": str(e)},
        )
...
async def store_vector(
    ...
    try:
        ...
        document = {"text": cleaned_text, "metadata": request.metadata}
        ...
        raise HTTPException(
            status_code=500, detail=f"خطا در ذخیره متن در پایگاه داده برداری: {str(e)}"
        )
    ...
    **نمونه خروجی:**
    ...
    try:
        ...
        get_vector_store().update_document(
            document_id=document_id, text=update_request.text, metadata=doc["metadata"]
        )
