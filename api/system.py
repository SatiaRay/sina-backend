...
from database.models import Base, User, Wizard, CrawledDomain, CrawlJobs, Document, Chat, ChatHistory, Workflow, Instruction
...
class DatabaseExportImport:
    ...
    def export_database(self) -> str:
        ...
        try:
            ...
            metadata = {
                ...
                "total_mysql_records": sum(len(records) for records in mysql_data.values()),
                "total_chroma_records": sum(len(records) for records in chroma_data.values())
            }
            ...
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                # Add MySQL data
                zipf.writestr("mysql_data.json", json.dumps(mysql_data, indent=2, ensure_ascii=False, default=str))
                
                # Add ChromaDB data
                zipf.writestr("chroma_data.json", json.dumps(chroma_data, indent=2, ensure_ascii=False, default=str))
    ...
    def _export_chroma_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"satya_docs": {"collection_name": collection_name, "count": 0, "ids": [], "documents": [], "metadatas": [], "embeddings": []}}
    ...
    def import_database(self, file: UploadFile) -> Dict[str, Any]:
        ...
        try:
            ...
            if not os.path.exists(metadata_path):
                raise HTTPException(status_code=400, detail="Invalid export file: metadata.json not found")
            ...
            if not os.path.exists(mysql_path):
                raise HTTPException(status_code=400, detail="Invalid export file: mysql_data.json not found")
            ...
            if not os.path.exists(chroma_path):
                raise HTTPException(status_code=400, detail="Invalid export file: chroma_data.json not found")
    ...
    def _import_mysql_data(self, mysql_data: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
        """Import MySQL data"""
        db, db_url = self._get_database_connection()
                    ...
                    try:
                        ...
                        for record_data in records:
                            ...
                            for key, value in record_data.items():
                                if isinstance(value, str) and key in ['created_at', 'updated_at', 'last_login', 'email_verified_at', 'started_at', 'end_at', 'timestamp']:
                                    try:
    ...
    def _import_chroma_data(self, chroma_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_records": sum(data.get("count", 0) for data in chroma_data.values())
            }
...
async def get_export_status():
    ...
    try:
        if db_export_import.export_filename and db_export_import.temp_dir:
            file_path = os.path.join(db_export_import.temp_dir, db_export_import.export_filename)
